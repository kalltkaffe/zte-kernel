...
/*histstory:
 ...
 --------        ----          ----------------------            -------------------------------
2011-01-19 yangxs  add wake lock when remove heaset         ZTE_HS_YXS_20110119
2010-12-07 yangxs  sidekey interrupt wakeup when fm is on   ZTE_AUDIO_YXS_20101217
...
static bool fm_status = false;
void report_fm_closed(void); /* @rpc_server_handset.c */
#endif
...
int gpio_to_irq(unsigned gpio); /* declaration, the definition is in generic_gpio.c */

enum boolean {
	NO, /* disable power management wakeup */
	YES /* enable power management wakeup */
};

static int set_gpio_wake(unsigned int gpio, unsigned int on)
{
	int err, irq;

	err = irq = gpio_to_irq(gpio);
	if (err < 0) return err;

	err = set_irq_wake(irq, on);
	if (err) {
		pr_err("[FM]set_gpio_wake failed for gpio %d, irq %d\n", gpio, irq);
	}
	return err;
}

static void set_sidekey_gpio_wake(unsigned int on)
{
	int err;

	/* definition of sidekey gpio is in file board-zte-[boardname]-keypad.c */
	#if defined(CONFIG_MACH_V9)
	 #define GPIO_VOLUMEUP 41
	 #define GPIO_VOLUMEDOWN 37
	#elif defined(CONFIG_MACH_BLADE) || defined(CONFIG_MACH_AMIGO)
	 #define GPIO_VOLUMEUP 37
	 #define GPIO_VOLUMEDOWN 41
	#endif

	/* Here is the code with high portability.
	 * If u emphasize efficiency more, pls use code with higher efficiency, such as:
	 * err = set_irq_wake(MSM_GPIO_TO_INT(GPIO_VOLUMEUP), on);
	 * err = set_irq_wake(MSM_GPIO_TO_INT(GPIO_VOLUMEDOWN), on);
	 */
	err = set_gpio_wake(GPIO_VOLUMEUP, on);
	if (err < 0) {
		pr_err("[FM]set_gpio_wake volup failed!!!!\n");
	}

	err = set_gpio_wake(GPIO_VOLUMEDOWN, on);
	if (err < 0) {
		pr_err("[FM]set_gpio_wake voldown failed!!!!\n");
	}
}
#endif /* CONFIG_MACH_V9 */
...
static int si4708_set_frequency(int frequency)
{
	...
	msleep(SI4708_TUNE_DELAY);    /*seek time*/
